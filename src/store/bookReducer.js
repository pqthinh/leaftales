...
export const bookSlice = createSlice({
  ...
  reducers: {
    ...
    readBookDetail: (state, action) => {
      state.currentBook = action.book
    },
    getBook: (state, action) => {
      console.log('action in getChapter: ---> ', action.payload.chapter_id)
      state.currentPage = action.currentPage || 0
      state.currentChapter = action.chapter_id || '1'
      state.chapterContent = action.content
    },
    ...
    searchBooks: (state, action) => {
      state.searchResults = state.books.filter(
        book =>
